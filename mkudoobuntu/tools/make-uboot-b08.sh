...
args=("$@")
ubootdir=${args[0]}
binarydir="$(dirname "$(pwd)")/boards/seco-b08"
targetfile="uboot.imx"

cd $ubootdir
ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- make clean
ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- make seco_b08_config
ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- ./compile_b08.sh -c
cd -
